...
class Box < BasePageObject
  ...
  def self.add_account(account)
    ...
    login_box_textfields[0].send_keys account[:login_st] if login_box_textfield_wait
    pass_box_textfields[1].send_keys account[:pass_st]   if pass_box_textfield_wait
    authorize_box_button_click                           if authorize_box_button_wait
    allow_access_box_button_click                        if allow_access_box_button_wait
    name_box_textfield_fill account[:storage]            if name_box_textfield_wait
